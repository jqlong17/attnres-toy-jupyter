...
class TinyTransformerLM(nn.Module):
    ...
    def __init__(self, config: Config, variant: str = "baseline") -> None:
        ...
        if variant not in {"baseline", "attnres"}:
            raise ValueError("variant must be 'baseline' or 'attnres'")
        self.config = config
        ...
        self.blocks = nn.ModuleList(
            [
                BaselineBlock(config) if variant == "baseline" else AttnResBlock(config)
...
def run_training(
    ...
    for step in range(steps):
        ...
        if model.variant == "attnres" and metrics:
